...
const props = defineProps<{ state: CigaretteHeroState; nickname?: string; streak?: number }>()
...
<template>
  <section class="cigarette-hero" :class="`is-${state}`">
    <div class="hero-copy">
      ...
      <h1>{{ title }}</h1>
      <p>{{ caption }}</p>
      <div v-if="streak!==undefined" class="hero-streak"><strong><AnimatedValue :value="streak" /></strong><span>天<br/>连续无烟</span></div>
    </div>
...
.hero-streak{display:flex;align-items:flex-end;gap:9px;margin-top:17px}.hero-streak strong{color:var(--green-900);font-size:62px;line-height:.86;letter-spacing:-.07em;font-variant-numeric:tabular-nums}.hero-streak span{color:#607269;font-size:11px;line-height:1.35}.is-extinguished{background:radial-gradient(circle at 80% 10%,rgba(213,240,219,.75),transparent 36%),linear-gradient(145deg,#e8f5eb,#fff 66%)}.is-weak{background:radial-gradient(circle at 86% 18%,rgba(245,224,190,.34),transparent 32%),linear-gradient(145deg,#f0f6f1,#fff 65%)}
</style>

...
const timeRange=ref<TimeRange>('30d');const metric=ref<'smoking'|'craving'>('smoking');const chartType=ref<'line'|'bar'>('line');const selectedDay=ref<HeatmapPoint|null>(null)
    ...
    <template v-else>
      ...
      <template v-if="activeSummary?.statistics&&activeSummary.smokingProfile">
        ...
        <section v-else-if="activeSection===1" class="tab-content">
          <section class="card heatmap-card"><div class="card-heading"><h2>戒烟日历</h2><span>{{(coverage*100).toFixed(0)}}% 覆盖</span></div><SmokingHeatmap :points="heatmapPoints" @select="selectedDay=$event" /><div v-if="selectedDay" class="day-detail"><div><b>{{selectedDay.date}}</b><span>{{selectedDay.status==='missing'?'未记录':selectedDay.cigarettes===0?'无烟':`${selectedDay.cigarettes} 支`}}</span></div><p>烟瘾 {{selectedDay.craving??'—'}} / 5</p><small v-if="selectedDay.note">{{selectedDay.note}}</small></div></section>

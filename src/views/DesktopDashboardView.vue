...
<template>
  <DesktopLayout>
    ...
    <div v-else-if="activeSummary&&stats&&activeSummary.smokingProfile" class="dashboard-grid">
      ...
      <section class="metrics-panel"><div class="panel-heading"><div><span>今日概览</span><h1>{{activeSummary.member.nickname}}的戒烟进展</h1></div><span class="role-chip">{{familyStore.isQuitter?'我的数据':'家庭戒烟者'}}</span></div><div class="desktop-stats"><StatCard label="连续无烟" :numeric-value="stats.currentStreak" suffix=" 天" icon="calendar-o" tone="accent" featured/><StatCard label="今日状态" :value="todayLabel" icon="clock-o"/><StatCard label="累计少吸" :numeric-value="stats.savedCigarettes" suffix=" 支" icon="bar-chart-o" tone="borderless"/><StatCard label="累计节省" :numeric-value="stats.savedMoney" prefix="¥" icon="balance-o" tone="borderless"/></div></section>

-- Lightweight family chat patch for an existing CloudBase PostgreSQL database.
-- Target environment contract: auth.users.id and public.profiles.id are bigint.
-- Run database_cloudbase.sql alone for a fresh install; use this patch only when
-- adding messages to an older CloudBase core schema.
...
begin
  ...
  if v_profile_id_type is null then
    raise exception 'CloudBase chat preflight failed: public.profiles.id was not found';
  end if;

  if v_profile_id_type <> 'bigint' then
    raise exception
      'CloudBase chat preflight failed: expected public.profiles.id bigint, found %',
      v_profile_id_type;
  end if;

  if pg_catalog.to_regprocedure('public.current_user_id()') is null then
    raise exception 'CloudBase chat preflight failed: public.current_user_id() was not found';
  end if;

  if pg_catalog.to_regprocedure('public.is_family_member(uuid)') is null then
    raise exception 'CloudBase chat preflight failed: public.is_family_member(uuid) was not found';
  end if;
end;
$$;
...
-- No Supabase publication statement is installed here. CloudBase PostgreSQL
-- realtime transport has not been assumed compatible with Supabase channels.
